  ...
  return (
    <Link href={`/kegs/${keg.id}`} className="block rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-slate-300">
      <div className="flex items-center justify-between gap-3">
        <h3 className="font-semibold">{keg.kegId ?? keg.id}</h3>
        <StatusBadge status={keg.currentStatus} />
      </div>
      <p className="mt-3 text-xs font-semibold tracking-[0.12em] text-slate-500">CONTENTS</p>
      <p className="text-sm text-slate-700">{contents}</p>
      ...
      <p className="mt-2 text-xs font-semibold tracking-[0.12em] text-slate-500">INTENDED LOCATION</p>
      <p className="text-sm text-slate-700">{keg.intendedLocation ?? "Not set"}</p>
    </Link>
  );
}
